...
for(K in 1:length(tt_boot$basis)){
    ...
    auc[K] <- pROC::roc(cohort_full$InHospMortality, phat)$auc
    
}


performance_mat <- matrix(c(1:length(tt_boot$basis_mats),
                            brier,
                            auc), ncol=3,
                          dimnames=list(NULL, c("K", "Brier", "AUC")))

plot(performance_mat[,2], performance_mat[,3])
plot(performance_mat[,1], performance_mat[,2])
plot(performance_mat[,1], performance_mat[,3])
...
for (fold_no in 1:max(cv_data$fold)) {
  ...
  # for(K in 1:length(tt_boot$basis)){
        ...
        brier[[fold_no]][K] <- sum((phat - cohort_full$InHospMortality)^2) / nrow(boot_data)
        auc[[fold_no]][K] <- pROC::roc(cohort_full$InHospMortality, phat)$auc
